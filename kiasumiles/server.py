...
def kiasumiles_list_cards() -> dict:
    """List all Singapore credit cards in the KiasuMiles database. Call this before kiasumiles_configure so you can match the user's cards to valid card_ids. Present the list grouped by bank in plain language — never show card_id values to the user."""
    cards = _loader.cards()
    return {
        "cards": [
            {
...
def kiasumiles_configure(cards: list[str]) -> dict:
    """
    Save the user's credit card wallet. Call kiasumiles_list_cards first to get valid card_ids,
    then pass the matched card_ids here. If the user mentions Amaze, include "amaze" in the list.
    After saving, confirm which cards were saved in plain language and tell the user they can now
...
def kiasumiles_lookup(
    ...
    """
    Find the best credit card to use at a Singapore merchant to earn the most miles.
    Use this whenever the user asks which card to use, what to tap, or which card earns
    the most miles — even if they don't mention KiasuMiles by name.
    ...
    recommendations = rank_cards(record.mcc, wallet_cards, wallet_has_amaze, top_n)
